...
class residual_l2(BaseReward, namespace="mimic_lite"):
    """Penalize residual PD actions. Compute is already negative; YAML weight is positive."""

    def _initialize_impl(self, **kwargs):
        self.action_manager = self.env.input_managers["action"]

    def _compute(self):
        action = self.action_manager.applied_action
        return -action.square().sum(dim=-1, keepdim=True)
